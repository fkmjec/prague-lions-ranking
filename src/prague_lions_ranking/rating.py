...
def calculate_attendance(matches: list[Match]) -> dict[str, dict[str, int]]:
    """Calculate practice attendance and game counts for all players.

    Returns:
        Dict mapping username to {'practices': int, 'games': int}
    """
    player_stats: dict[str, dict[str, set | int]] = defaultdict(
        lambda: {"practice_dates": set(), "games": 0}
    )
...
def calculate_ratings(db: Session) -> dict[str, dict]:
    ...
    for name in player_names:
        ...
        player_attendance = attendance.get(name, {"practices": 0, "games": 0})
